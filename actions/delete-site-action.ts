...
export async function deleteSite(siteId: string): Promise<{ success: boolean; message: string; data?: any }> {
  try {
    ...
    if (noIsNotFound(error)) {
      return { success: false, message: error?.message || "Failed to delete site" };
    }

    return { success: true, message: "Website deleted successfully", data: true };
  } catch (error: any) {
    return { success: false, message: error?.message || "An error occurred while deleting the site" };
  }
}
